...
marcas_modelos = []
quilometro_por_litro = []

for carros in range(0, 5):
    print(f"Veículo {carros + 1}")
    pergunta = input("Nome: ")
    marcas_modelos.append(pergunta)
    km = float(input("Km por litro: "))
    quilometro_por_litro.append(km)

for c in range(0,5):
    print(f"{c + 1} - {marcas_modelos[c]:<10} - {quilometro_por_litro[c]:^5.1f} - {1000 / quilometro_por_litro[c]:>5.1f} litros - R$ {(1000 / quilometro_por_litro[c]) * 2.25:^5.2f}")